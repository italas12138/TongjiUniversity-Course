...
public:
    ...
    vector<FuncEnterTable> retFunTable;            // 返回给目标代码的函数表
    _MediateCodeGen _code;                          // 中间代码
    _Var* _LookUpVar(string _varName);              // 查找变量
    _Fun* _LookUpFunc(string _funName);             // 查找函数
    int _nowLevel;                                  // 当前层级
    ofstream quaterFile;                            // 四元式输出文件


    // 判断函数参数是否匹配
    bool _FunParMatch(list<string>& _argumentList, list<__DataType>& _parameterList);
    ...
    vector<FuncEnterTable>* getFunTable();
    vector<_Quaternary> getMediateCode();
};

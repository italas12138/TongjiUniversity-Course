...
void wordParse::NFABuild(string grammarFileName)
{
    ...
    for (int i = 0; i < proNums; i++)
    {
        ...
        // 构造NFA的转移，stateTransfer[i][j]表示状态i接受字符j后可能到达的状态集合
        if (right.length() > 1)
        {
...
void wordParse::NFAConvert()
{
    ...
    //各状态如何扩展顺序任意，这里用栈存储状态，后入栈的状态先扩展
    stack<NFAstateSet> s;
    ...
    while (!s.empty())
    {
        ...
        {
            //遍历每个终结符，当它到来时，当前状态内的每个非终结符的转移存放到下一个状态集里
            ...
            {
                //如果下一个状态集不空，并且当前DFA中还未存在这个状态集，将其加入DFA中
                s.push(nextStateSet);
...
void wordParse::work()
{
    ...
    cout << "Please input the testfileName:" << endl;
    cin >> testFileName;
    fileSource = fopen(testFileName.c_str(), "r+");
    if(fileSource == nullptr){
        _errorLog("Failed to open the '" + testFileName + "'");
        exit(-1);
    }
    scanFile();
    fclose(fileSource);

...
void _GrammarParse::_SemanticAnalyse(list<Word>& _lexicalResult)
{
        ...
        else
        {
            ...
            */
            ...
            case 17:
            {
                ...
                if (!_f)
                {
                    _errorLog(string("语法错误：第") + to_string(_linesCnt) + "行，未声明的函数" + _name->_name);
                    exit(-1);
                }
            ...
            case 18:
            {
                ...
                {
                    _errorLog(string("语法错误：第") + to_string(_linesCnt) + "行，变量" + _name->_name + "未声明");
                    exit(-1);
                }
                _Value* _value = new _Value(_nowPro._left);
    ...
    if (!_acc)
    {
        _errorLog("Failed to Analyse. The program has a syntax error\n");
        exit(-1);
    }
    else
    {
        ...
        // 输出语义分析内容
        quaterFile.open(string("InterMediateCode.txt"), ios::out);
        if(!quaterFile.is_open())
        {
            _errorLog("InterMediateCode.txt open error!\n");
            exit(-1);
        }
        for(int i = 0; i < _code._quaCode.size(); i++)
        {
            quaterFile << i << " (" << _code._quaCode[i]._opt << "," <<
             _code._quaCode[i]._rs << "," << _code._quaCode[i]._rt << "," << _code._quaCode[i]._rd << ")"<< endl;
...
//addPart
vector<FuncEnterTable>* _GrammarParse::getFunTable()
{
    for(auto& fun : _funTable)
        retFunTable.push_back({fun._enterPoint, fun._funName});
    return &retFunTable;
}

vector<_Quaternary> _GrammarParse::getMediateCode()
{
    return _code._quaCode;
}
...
void WordAnalyze::output(ofstream& file)
{
    ...
    file << "词法分析结果" << endl << endl;
    for (auto it = Word_table.begin(); it != Word_table.end(); it++)
    {
        file << "字符值:" << setiosflags(ios::left) << setw(6) << it->first.first
            << "符号类别:" << setw(6) << it->first.second
            << "  位置:" << it->second.first << "," << it->second.second << endl;
        cout << "字符值:" << setiosflags(ios::left) << setw(6) << it->first.first
            << "符号类别:" << setw(6) << it->first.second
            << "  位置:" << it->second.first << "," << it->second.second << endl;
    }
    return;
